...
module tt_um_stoch_alu (
    ...
    wire [7:0] operand_a_ext = {4'b0, op_b};
    wire a_stream = (ui_in > lfsr);
    wire b_stream = (operand_a_ext > lfsr);
    ...
    wire [7:0] exact_add = ui_in + operand_a_ext;
    wire [7:0] exact_sub = ui_in - operand_a_ext;
    wire [11:0] exact_mul_full = ui_in * operand_a_ext;
    wire [7:0] exact_mul = exact_mul_full[7:0];
    wire [7:0] exact_and = ui_in & operand_a_ext;
    wire [7:0] exact_or  = ui_in | operand_a_ext;
    wire [7:0] exact_xor = ui_in ^ operand_a_ext;
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            sample_counter <= 6'd0; pop_count <= 7'd0;
            final_result <= 8'd0; sampling_done <= 1'b0;
            ...
            if (sample_counter < 6'd63) begin
                ...
                final_result <= {pop_count[6:0], 2'b00};
                sampling_done <= 1'b1;
                if ((pop_count < 7'd10) || (pop_count > 7'd54))
                    confidence_flag <= 1'b1;
                else confidence_flag <= 1'b0;
                sample_counter <= 6'd0;
                pop_count <= 7'd0;
                sampling_done <= 1'b0;
            end
